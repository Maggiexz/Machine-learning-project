...
#Single Variable Classification shiny app
...
#Multi-Variable Classification shiny app
...
#Clustering shiny app

ui <- fluidPage(
  titlePanel("Machine Learning Visualization"),
  sidebarLayout(
    sidebarPanel(
      selectInput("Clustering_Type", "Clustering Type", choices = c("Hierarchical Clustering", "K-means Clustering")),
      conditionalPanel(
        condition = "input.Clustering_Type == 'Hierarchical Clustering'",
        selectInput("Plot_Type", "Plot Type", choices = c("Tree Plot", "CH index plot and WSS plot"))
      ),
      conditionalPanel(
        condition = "input.Clustering_Type == 'K-means Clustering'",
        selectInput("Plot_Type", "Plot Type", choices = c("CH index plot and WSS plot", "Scatter Plot"))
      )
    ),
    mainPanel(
      plotOutput(outputId = "CH_index_WSS_plot"),
      plotOutput(outputId = "hierarchical_clustering_plot"),
      plotOutput(outputId = "CH_index_ASW_plot"),
      plotOutput(outputId = "Kmeans_Scatter_Plot")
    )
  )
)
...
server <- function(input, output, session) {
  observe({
    #
    if (input$Clustering_Type == "Hierarchical Clustering") {
      
      # Define server logic for hierarchical clustering and Plot Type
      output$CH_index_WSS_plot <- renderPlot({
        # Create CH index and WSS plots
        # Make sure to replace the placeholders with actual code
        # Example code:
        CHIndexPlot <- ggplot(crit.df, aes(x=k, y=CH_index)) +
          geom_point() + geom_line(colour = "red") +
          scale_x_continuous(breaks = 1:10, labels = 1:10) +
      ...
      output$hierarchical_clustering_plot <- renderPlot({
    ...
    } else if (input$Clustering_Type == "K-means Clustering") {
      # Define server logic for k-means clustering and Plot Type
      output$CH_index_ASW_plot <- renderPlot({
        # Create CH index and ASW plots
        # Make sure to replace the placeholders with actual code
        # Example code:
        CHIndexPlot <- ggplot(kMeansMetrics, aes(x = k, y = CH_index)) +
          geom_point() + geom_line(colour = "red") +
      ...
      output$Kmeans_Scatter_Plot <- renderPlot({
        # Create k-means scatter plot
        # Make sure to replace the placeholders with actual code
        # Example code:
        plotList <- list()
        kRange <- 2:5
